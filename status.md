...
## 🔴 2026-09-19 CTFd API 全量同步 — 整盤棋變了，請先讀這段

planner 用使用者提供的 CTFd API token 重抓全平台資料，結果與本檔先前的記載**差異巨大**：

### 1. 題目數 14 → **25 題**（多了 11 題，且出現全新的 Web 分類）

新題：Web/Ghosttenant(984)、Web/Sports(951)、Web/JSFBox 4(324)、
Pwn/aegis_factory(996)、Pwn/SimpleHttpServer(740)、Pwn/Pool93(676)、
Misc/5P(676)、Crypto/strange system 1-3(271/559/471)、Rev/sn4k3_g4m3(271)。
**13 個資料夾與 README 已建立、附件已下載**（見 README.md 總表）。

### 2. ⚠️ 分數是**動態計分**，我們追的題目全部大幅貶值

| 題目 | 舊記錄 | 實際現值 | 變化 |
|---|---|---|---|
| False_Continuity | 804 | **100** | −704 |
| arbitragedb | 711 | **100** | −611 |
| aegis_asterism | 600 | **100** | −500 |
| Slime | 975 | **471** | −504 |
| AI_Challenge | 936 | **471** | −465 |

**本檔先前所有基於舊分數的優先序判斷都已失效。**
目前投入中的兩題（arbitragedb 100、Slime 471）已是低價值標的，
而 996/984/951/740 分的高價值題**完全沒人碰**。是否改變分派由使用者決定。

### 3. 得分現況

已解 **6 / 25 = 600 分**（2026-09-19 extraction-2 解出，待提交）；未解總分 **8461 分**。
...
呼叫端 `0x4c94` 的參數：`rcx ← -0x58`、`r8 ← rdi ← -0x78`。

**三個 varint 的完整解碼順序（已逐一確認）**：

| 解碼點 | 存到 | 用途 | 對應 gen_poc.py |
|---|---|---|---|
| `0x4b65` `sub_44bb` | `-0x80` | **被 `0x4c43` 檢查 `<= 0x80`** | **A**（第一個） |
