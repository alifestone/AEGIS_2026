...
- **下一步**（給接手者）：
  1. **48 張單張仍是最大的未探索面** —— 48 對 + 48 單張的對稱結構應該有意義。
     我掃過單張的英文字（除上述假象外）沒有其他命中，但沒做更深的分析。
  2. 差異字元的順序：試「差異在原始大頁面的絕對座標」而非紙屑內相對座標
  3. ⚠️ 用 fc_ocr.pkl 做任何逐字元推理前，先確認該張的 180° 方向是對的
     （修正 8 顯示至少有一張是反的，可能不只一張）

- **Flag**：—

### Jurassic_Time_Capsule
- **狀態**：進行中（**重大突破：確認在台灣**）
- **提交紀錄**：0 / 10（尚未提交任何答案，⚠️ 上限 10 次）
- **Flag**：—
...
1. 判讀出 2.5km 指標的完整園區名稱（鯨魚圖示暗示海洋／親水主題公園）
2. 用「恐龍園區 + 該公園相距 2.2km」的幾何關係反推河段
3. **確認雕像在 2019 年存在、現已移除**（題敘明示雕像已不在）
4. 鎖定後用 Google Earth 歷史影像比對草地上的雕像基座位置
